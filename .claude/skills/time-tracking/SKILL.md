...
| `--blocks` | show each block's clock times — WHEN the time was spent |
...
| `--min H` | roll rows under H hours into `(other)` (default 0.05) |
...
- Always state the gap value and the date range alongside any total, and whether it is
  claude-only or `claude+git` (the CSV's `sources` column records this).
...
# when was the time spent? block-level clock times for a day
